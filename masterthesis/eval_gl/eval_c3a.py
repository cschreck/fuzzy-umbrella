...
from masterthesis.models import HeadingEstimator
...
def run(train, test_trajs, after_x=1/2, estimator=HeadingEstimator(), **kwargs):
    estimator = estimator.fit(train)

    ranks = list()
    for route in test_trajs:
        partial_end = int(len(route) * after_x)
        preds = estimator.predict_proba(route[0:partial_end])
